...
task fetch_stock_info: :environment do
  ...
  list_of_companies.each do |_c|
    # API call variables
    weekly_timeseries = company.stock(symbol:).timeseries(outputsize: 'full', adjusted: true,
                                                          type: 'weekly').output['Weekly Adjusted Time Series']
    monthly_timeseries = company.stock(symbol:).timeseries(outputsize: 'full', adjusted: true,
                                                           type: 'monthly').output['Monthly Adjusted Time Series']
    business_data = company.stock(symbol:).fundamental_data.overview
  end
end
